...
/* 发布者函数 */
extern "C" int publisher_main(int domainId, int sample_count)//0 0
{
	...
	for (count = 0; (sample_count == 0) || (count < sample_count); ++count)
	{
		...
		data += "   Hello World!!!";
		instance->color = const_cast<char*>(data.c_str());
		string MD5_data = GetDataMD5(instance->color);//计算MD5
		instance->data = const_cast<char*>(MD5_data.c_str());

		if (now - lastTime > 0)//一秒钟执行一次
		{
			if (retcode != RETCODE_OK)
			{
			...
			else
			{
				...
				cout << "handling capacity = " << (count - pre_cout) * instance->StructSize() * 8 << endl << endl;
			}
			pre_cout = count;
			lastTime = now;
		}
		instance->num = (count - pre_cout);

		retcode = UserDataType_writer->write(*instance, instance_handle);//发送数据
...
int main(int argc, char* argv[])
{
	...
	thread send(publisher_main, domain_id, sample_count);
	send.join();
